...
| `연결된 멤버 프로필을 members.yml에서 찾을 수 없습니다` | `member_id`가 `_data/members.yml`의 `id`와 다름(오타 포함) | Table Editor에서 `member_id`를 실제 `id`와 정확히 같게. 3장 |
| `이 프로필 유형은 관리자 콘텐츠 편집기에서만 수정할 수 있습니다` | `member_id`가 `pi-keunwoo-jang`으로 되어 있음. PI 항목은 본인 편집 대상이 아님 | 다른 `id`로 바꾸거나 그 계정을 `admin`으로 |
| My profile 메뉴가 없음 | `member_id`가 비어 있음 | 정상입니다. 열람만 됩니다. 프로필 수정이 필요하면 `member_id`를 채웁니다. 3장 |
...
| 페이지 원본 편집 | O | X |
| 본인 프로필 수정 | `member_id` 연결 시 O | `member_id` 연결 시 O |

`member`로 로그인하면 관리자와 같은 화면이 모두 보이되 입력 칸이 잠깁니다. 내용을 읽고 복사할 수는
있으므로, 고칠 부분을 관리자에게 알려 주는 용도입니다. 화면 위에 읽기 전용 안내가 뜹니다.

읽기 전용은 화면에서만 막는 것이 아닙니다. 서버가 저장 계열 요청을 전부 거부하므로 브라우저를
조작해도 수정할 수 없습니다. 열람을 허용해도 새로 공개되는 정보는 없습니다. 저장소가 공개라
모든 파일을 이미 누구나 GitHub에서 볼 수 있습니다.

수정 가능한 것은 자기 항목의 10개 필드뿐입니다: `name_en`, `name_ko`, `email`, `github`, `cv`,
`website`, `affiliation`, `education`, `research_areas`, `bio`. 서버에서 강제하므로 다른 항목이나
다른 사람은 건드릴 수 없습니다. `member_id`가 비어 있으면 열람만 가능하고 My profile은 나오지
않습니다.
...
`member` 계정에도 같은 7개 그룹이 보이며, 입력 칸이 잠긴 상태로 열립니다. `member_id`가 연결된
계정은 맨 위에 My profile이 하나 더 붙고 그 화면만 수정할 수 있습니다.
